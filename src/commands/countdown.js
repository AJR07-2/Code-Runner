...
module.exports = async (message) => {
  ...
  if (arr.length >= 5) {
    time += parseInt(arr[1]) * 60 * 60 + parseInt(arr[2]) * 60 + parseInt(arr[3]);
    ...
    let sent = await message.channel.send(
      new Discord.MessageEmbed()
		...
		startCountdown(time, sent, countdownMessage);
...
function startCountdown(time, message, countdownMessage) {
  let [seconds, minutes, hours] = parseSeconds(time), colour = "0000ff";
  time--;
  if (time === 0) {
    message.edit(
      new Discord.MessageEmbed()
        .setColor("00ff00")
        .setTitle(`COUNTDOWN WITH NAME OF "${countdownMessage}" HAS COMPLETED!!!`)
    );
		return;
  }
  ...
  setTimeout(() => {
    startCountdown(time, message, countdownMessage);
  }, 1000);
}